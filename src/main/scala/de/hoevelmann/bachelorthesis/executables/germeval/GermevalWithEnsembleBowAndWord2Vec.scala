...
import de.hoevelmann.bachelorthesis.modelling.transformers.{FastTextSentenceVector, GermanStemmer, LanguageToolTransformer, WordCounter}
...
/*
object GermevalWithEnsembleBowAndWord2Vec {
  ...
  def execute(spark: SparkSession = SparkSession.builder().master("local[*]").getOrCreate(),
    ...
    val word2Vec = new FastTextSentenceVector().setInputCol("input").setOutputCol("sentenceVectors")
    val nGramStep: NGram = new NGram().setInputCol("stems").setOutputCol("ngrams").setN(2)
    ...
    val bowVectorAssembler: VectorAssembler = new VectorAssembler().setInputCols(Array("idfs", "liwcFeatures", "languageToolFeatures", "wordCounts")).setOutputCol("assembledIdfFeatures")
    val word2VecVectorAssembler: VectorAssembler = new VectorAssembler().setInputCols(Array("sentenceVectors", "liwcFeatures", "languageToolFeatures", "wordCounts")).setOutputCol("assembledWord2VecFeatures")
    ...
    def sentimentWord2VecLayers: Array[Int] = Array(185, 50, 3)

    def relevanceBowLayers: Array[Int] = Array(1000) ++ intermediateLayers ++ Array(2)

    def relevanceWord2VecLayers: Array[Int] = Array(185, 50, 2)
    ...
    val sentimentMLPBowTrainer = new MultilayerPerceptronClassifier().setPredictionCol("afjdslk").setProbabilityCol("t54").setLabelCol("sentiment").setFeaturesCol("topFeatures")
      .setLayers(sentimentBowLayers).setBlockSize(128).setSeed((Math.random() * 100000).toLong).setMaxIter(numIterations).setRawPredictionCol("bowClassifierPrediction")
    val sentimentMLPWord2VecTrainer = new MultilayerPerceptronClassifier().setPredictionCol("fjsdfaoi").setProbabilityCol("544").setLabelCol("sentiment").setFeaturesCol("assembledWord2VecFeatures")
      .setLayers(sentimentWord2VecLayers).setBlockSize(128).setSeed((Math.random() * 100000).toLong).setMaxIter(numIterations).setRawPredictionCol("word2VecClassifierPrediction")
    ...
    val relevanceMLPWord2VecTrainer = new MultilayerPerceptronClassifier().setPredictionCol("fsdfjsjojlk").setPredictionCol("345").setLabelCol("relevanceLabel").setFeaturesCol("assembledWord2VecFeatures")
      .setLayers(relevanceWord2VecLayers).setBlockSize(128).setSeed((Math.random() * 100000).toLong).setMaxIter(numIterations).setRawPredictionCol("word2VecClassifierPrediction")
    ...
    val sentimentPipelineMLP = new Pipeline().setStages(Array(languageTool, germanStemmer, word2Vec, word2VecVectorAssembler, nGramStep, hashingTermFrequencies, idf, bowVectorAssembler, sentimentChiSqSelector,
      sentimentMLPBowTrainer, sentimentMLPWord2VecTrainer, ensembleAssembler, ensembleSentimentMLPTrainer))
    val relevancePipelineMLP = new Pipeline().setStages(Array(languageTool, germanStemmer, word2Vec, word2VecVectorAssembler, nGramStep, hashingTermFrequencies, idf, bowVectorAssembler, relevanceChiSqSelector,
      relevanceMLPBowTrainer, relevanceMLPWord2VecTrainer, ensembleAssembler, ensembleRelevanceMLPTrainer))
